...
function test(app) {
  it('should utilize qvalues in negotiation', done => {
    request(app).get('/').set('Accept', 'text/html; q=.5, application/json, */*; q=.1').expect({ message: 'hey' }, done)
  })

  it('should allow wildcard type/subtypes', done => {
    request(app).get('/').set('Accept', 'text/html; q=.5, application/*, */*; q=.1').expect({ message: 'hey' }, done)
  })
