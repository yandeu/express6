  ...
  children.forEach(child => {
    ...
    if (Array.isArray(child)) appendChildren(element, child)
      ...
      if (typeof c !== 'undefined') {
        ...
        else element.appendChild(c.nodeType == null ? __jsx.document.createTextNode(c.toString()) : c)
      }
    }
  })
}
...
const hNS = (tag: string) => __jsx.document.createElementNS('http://www.w3.org/2000/svg', tag) as SVGElement
...
export const h = (tagNameOrComponent: any, props: any, ...children: any) => {
  ...
  const element =
    tagNameOrComponent === 'svg'
      ...
      : (__jsx.document.createElement(tagNameOrComponent) as HTMLElement)
